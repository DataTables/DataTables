...
g[e]=="desc"?d.sSortJUIDesc:c.sSortingClassJUI)}})}}});k.render={number:function(a,b,c,d,e){return{display:function(f){if("number"!==typeof f&&"string"!==typeof f)return f;var h=0>f?"-":"",f=Math.abs(parseFloat(f)),g=parseInt(f,10),f=c?b+(f-g).toFixed(c).substring(2):"";return h+(d||"")+g.toString().replace(/\B(?=(\d{3})+(?!\d))/g,a)+f+(e||"")}}}};g.extend(k.ext.internal,{_fnExternApiFunc:Mb,_fnBuildAjax:ra,_fnAjaxUpdate:kb,_fnAjaxParameters:tb,_fnAjaxUpdateDraw:ub,_fnAjaxDataSrc:sa,_fnAddColumn:Ga,
